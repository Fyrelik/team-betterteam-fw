...
hasSamba=0
dumpSamba(){
	grep -q samba /etc/extra_modules && hasSamba=1
	tag 'samba' "$hasSamba"
}

optAgentVersion=0
dumpOptAgent(){
	optAgentVersion=`cut -d '=' -f 2 /opt/version`
	beginTag 'optAgent'
	tag 'version' "$optAgentVersion"
	endTag 'optAgent'
}

hasOpenSSL=0
dumpOpenSSL(){
	grep -q openssl /etc/extra_modules && hasOpenSSL=1
	tag 'openssl' "$hasOpenSSL"
}

setWlanInterface() {
	median=$(tdb get Wireless STA_DEV_ms)
	[ -z $median ] && median=$(tdb get Network ETHERNET_DEV_ms)
} 
...
"
	...
	beginTag 'common'
	setWlanInterface
	ipv4address=$(ifconfig $median | grep "inet addr:" | cut -d ':' -f2 | cut -d ' ' -f1)
	tag eth0Ipv4Addr "$ipv4address"
	dumpPeripheral
